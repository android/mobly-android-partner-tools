...
def _generate_test_result_xml(
    ...
    with open(mobly_summary) as f:
        ...
        for entry in summary:
            ...
            # Parse user data
            if entry_type == records.TestSummaryEntryType.USER_DATA.value:
                for key, value in entry['properties']:
                    if key == 'apa_suite_name':
                        apa_suite_name = value
                    if key == 'apa_suite_plan':
                        apa_suite_plan = value
                    if key == 'apa_suite_version':
                        apa_suite_version = value

    xml_root = ElementTree.Element(
        'Result',
        {
            'suite_name': apa_suite_name,
            'suite_plan': apa_suite_plan,
            'suite_version': apa_suite_version,
    ...
    # TODO: support execution of more than one module
